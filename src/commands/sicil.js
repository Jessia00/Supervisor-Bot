...
data = data.map((x) => `
#${x.id} ${x.active ? emojis.mark : emojis.cross} **[${x.type}]** ${moment(x.date).format("LLL")} tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış.\n`).join("─────────────────");
