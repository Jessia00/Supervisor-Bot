...
module.exports = {
    conf: {
      ...
      name: "kullanıcıbilgi",
      help: "kullanıcıbilgi",
      description: "Kullanıcı bilginizi gösterir",
    },
...
`);
  if (üye.hasPermission("ADMINISTRATOR") || Ayarlar.registration.register.some(x => üye.roles.cache.has(x))) 
...
`• Toplam kayıt: ${registerData ? registerData.top : 0} • Erkek kayıt : ${registerData ? registerData.erkek : 0} • Kadın kayıt : ${registerData ? registerData.kız : 0}`)
  ...
  if (üye.presence.activities.some(x => x.name == "Spotify" && x.type == "LISTENING")) {
    ...
    progressBar[calcul] = "🟢"
    message.channel.send(new MessageEmbed().setAuthor("Spotify bilgi", client.user.avatarURL()).setColor("#07c41d").setImage(`https://i.scdn.co/image/${presence.assets.largeImage.slice(8)}`).setDescription(
