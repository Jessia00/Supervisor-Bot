...
module.exports = {
  conf: {
    ...
    description: "Atılan ceza hakkında bilgi verir",
  },
  ...
  run: async (client, message, args, embed) => {
    if (isNaN(args[0])) return message.channel.send(embed.setDescription("Ceza ID'si bir sayı olmalıdır!"));
    const data = await penals.findOne({ guildID: message.guild.id, id: args[0] });
    if (!data) return message.channel.send(embed.setDescription(`${args[0]} ID'li bir ceza bulunamadı!`));
