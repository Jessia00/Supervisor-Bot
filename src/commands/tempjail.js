...
module.exports = {
  conf: {
    ...
    description: "Belirttiğiniz kullanıcıya temp-jail atar",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (conf.penals.jail.roles.some(x => member.roles.cache.has(x))) return message.channel.send(embed.setDescription("Bu üye zaten jailde!"));
    ...
    if (!message.member.hasPermission(8) && member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send(embed.setDescription("Kendinle aynı yetkide ya da daha yetkili olan birini jailleyemezsin!"));
    if (!member.manageable) return message.channel.send(embed.setDescription( "Bu üyeyi jailleyemiyorum!"));
    if (conf.penals.jail.limit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == conf.penals.jail.limit) return message.channel.send(embed.setDescription( "Saatlik jail sınırına ulaştın!"));
