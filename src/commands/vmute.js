...
module.exports = {
  conf: {
    ...
    description: "Belirttiğiniz kullanıcıya voice-mute atar",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (conf.penals.voiceMute.roles.some(x => member.roles.cache.has(x))) return message.channel.send(embed.setDescription("Bu üye zaten susturulmuş!"));
    ...
    if (conf.penals.voiceMute.limit > 0 && vmuteLimit.has(message.author.id) && vmuteLimit.get(message.author.id) == conf.penals.voiceMute.limit) return message.channel.send(embed.setDescription("Saatlik susturma sınırına ulaştın!"));
