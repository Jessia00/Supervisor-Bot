...
module.exports = {
  conf: {
    ...
    description: "Belirttiğiniz kullanıcıya mute atar",
  },
  ...
  run: async (client, message, args, embed) => {
    if (!message.member.hasPermission(8) && !conf.penals.chatMute.staffs.some(x => message.member.roles.cache.has(x))) return message.channel.send(embed.setDescription("Yeterli yetkin bulunmuyor!"));
    const member = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if (!member) return message.channel.send(embed.setDescription("Bir üye belirtmelisin!"));
    if (conf.penals.chatMute.roles.some(x => member.roles.cache.has(x))) return message.channel.send(embed.setDescription( "Bu üye zaten susturulmuş!"));
    const duration = args[1] ? ms(args[1]) : undefined;
    if (!duration) return message.channel.send(embed.setDescription(`Geçerli bir süre belirtmelisin!`));
    const reason = args.slice(2).join(" ") || "Belirtilmedi!";
    if (!message.member.hasPermission(8) && member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send(embed.setDescription("Kendinle aynı yetkide ya da daha yetkili olan birini susturamazsın!"));
    if (!member.manageable) return message.channel.send(embed.setDescription("Bu üyeyi susturamıyorum!"));
    if (conf.penals.chatMute.limit > 0 && muteLimit.has(message.author.id) && muteLimit.get(message.author.id) == conf.penals.chatMute.limit) return message.channel.send(embed.setDescription("Saatlik susturma sınırına ulaştın!"));
