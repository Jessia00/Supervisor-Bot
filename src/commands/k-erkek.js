...
.setThumbnail(uye.user.displayAvatarURL({ dynamic: true, size: 2048 })))
    await uye.roles.add(ayar.registration.manRoles)
    ...
    await isimler.findOneAndUpdate({ guildID: message.guild.id, userID: uye.user.id }, { $push: { names: { name: uye.displayName, yetkili: message.author.id, rol: ayar.registration.manRoles.map(x => `<@&${x}>`).join(" , "), date: Date.now() } } }, { upsert: true });
